...
table_default <- function(x,
  ...
  # --- Check if table wrapping is needed ---
  ...
  should_wrap <- auto_wrap && total_width > (terminal_width * wrap_threshold)

  if (should_wrap && length(col_widths) > 2) {
    # Determine wrapping strategy
    # Keep first column (usually row names/identifiers) and split remaining columns
    first_col_idx <- 1
    remaining_cols <- 2:length(col_widths)

    # Calculate available width for remaining columns
    first_col_width <- col_widths[first_col_idx] + 4 + n_space  # padding and separator
    available_width <- terminal_width * wrap_threshold - first_col_width

    # Group columns that fit within available width
    col_groups <- list()
    current_group <- c()
    current_width <- 0

    for (i in remaining_cols) {
      col_width <- col_widths[i] + n_space  # include separator
      if (length(current_group) == 0 || current_width + col_width <= available_width) {
        current_group <- c(current_group, i)
        current_width <- current_width + col_width
      } else {
        col_groups <- append(col_groups, list(current_group))
        current_group <- c(i)
    ...
    for (group_idx in seq_along(col_groups)) {
      current_cols <- c(first_col_idx, col_groups[[group_idx]])
      current_col_names <- col_names[current_cols]
      current_col_widths <- col_widths[current_cols]
      current_x_chars <- x_chars[, current_cols, drop = FALSE]

      # Adjust vertical borders for current group
      current_after_cols_idx <- integer(0)
      if (length(after_cols_idx) > 0) {
        # Map original indices to current group indices
        for (orig_idx in after_cols_idx) {
          new_idx <- match(orig_idx, current_cols)
          if (!is.na(new_idx) && new_idx < length(current_cols)) {
            current_after_cols_idx <- c(current_after_cols_idx, new_idx)
          }
        }
      }
      current_n_borders <- length(current_after_cols_idx)

      # Calculate total width for current group and centering
      current_total_width <- sum(current_col_widths) + (length(current_col_widths) - 1) * n_space + current_n_borders * (1 + nchar(border_char_v)) + 4

      # Calculate left padding for centering current group
      current_left_padding <- if (center_table && terminal_width > current_total_width) {
        paste0(rep(" ", floor((terminal_width - current_total_width) / 2)), collapse = "")
      } else {
        ""
      }

      # Create horizontal lines for current group
      create_horizontal_line_group <- function(connector_char_mid, connector_left = border_char, connector_right = border_char) {
        line <- paste0(connector_left, border_char)

        for (i in seq_along(current_col_widths)) {
          line <- paste0(line, paste0(rep(border_char, current_col_widths[i]), collapse = ""))

          if (i < length(current_col_widths)) {
            if (i %in% current_after_cols_idx) {
              line <- paste0(line,
                             border_char,
                             connector_char_mid,
      ...
      # Simple lines if no vertical borders
      if (current_n_borders == 0) {
        simple_width <- sum(current_col_widths) + max(0, length(current_col_widths) - 1) * n_space + 4
        simple_line <- paste0(rep(border_char, simple_width), collapse = "")

        top_line <- simple_line
        middle_line <- simple_line
        bottom_line <- simple_line
      } else {
        connectors <- getOption("tab_default")
        top_line <- create_horizontal_line_group(connectors$vb_top)
        middle_line <- create_horizontal_line_group(connectors$vb_mid)
        bottom_line <- create_horizontal_line_group(connectors$vb_bottom)
      }

      # Print group header if this is a continuation
      if (group_idx == 1) {
        ...
        # Print title if provided (only for first group)
        if (!is.null(title)) {
          title_width <- sum(current_col_widths) + (length(current_col_widths) - 1) * n_space + current_n_borders * (1 + nchar(border_char_v)) + 4
          title_line <- paste0(rep(border_char, title_width), collapse = "")

          cat(current_left_padding, title_line, "\n", sep = "")

          # Get title justification
          title_justify <- "center"
          if (!is.null(justify_cols) && is.list(justify_cols) && "title" %in% names(justify_cols)) {
          ...
          cat(current_left_padding, " ", formatted_title, " ", "\n", sep = "")
          cat(current_left_padding, top_line, "\n", sep = "")
        } else {
          cat(current_left_padding, top_line, "\n", sep = "")
        }
      } else {
      ...
      # Print headers for current group
      current_vb <- list(char = border_char_v, after = current_after_cols_idx)
      header_values <- stats::setNames(current_col_names, current_col_names)
      header_row <- format_row(header_values, current_col_widths,
                               ...
                               is_header = TRUE,
                               vb = current_vb)
      cat(current_left_padding, header_row, "\n", sep = "")
      cat(current_left_padding, middle_line, "\n", sep = "")

      # Print data rows for current group
      if (nrow(current_x_chars) > 0) {
        for (i in 1:nrow(current_x_chars)) {
          current_row <- stats::setNames(current_x_chars[i,], current_col_names)
          cat(current_left_padding,
              format_row(current_row, current_col_widths,
                         justify_cols = justify_cols,
                         n_space = n_space,
      ...
      cat(current_left_padding, bottom_line, "\n", sep = "")
    }

    # Display truncation message after all tables if needed
    if (truncated) {
    ...
    # --- Create a title line with no connectors ---
    ...
    if (!is.null(title)) {
      ...
      # Apply style to title if specified
      if (!is.null(style_colnames) && is.list(style_colnames) && "title" %in% names(style_colnames)) {
        style_fn <- style_colnames[["title"]]
        if (is.function(style_fn)) {
          context <- list(
