...
#' @param digits If NULL, it is set up by default. The default contains the options you get
#'   from `getOption("ct_digits")`. Else, you can provide it from a list or tweak it
#'   from the `options` list or use `tabhelpers_option`. It has following components:
#'   - `ex`: to adjust the digits by expected values.
#'   - `row_pct`: to adjust the digits by row percentages.
#'   - `col_pct`: to adjust the digits by column percentages.
#'   - `total_pct`: to adjust the digits by total percentages.
#'
...
cross_table <- function(data,
    ...
    default_digits <- getOption("ct_digits")

    if (is.null(digits)) {
        digits <- default_digits
    } else if (is.numeric(digits) && length(digits) == 1) {
        digits <- list(
            ex = digits,
            row_pct = digits,
            col_pct = digits,
    ...
    # Function to format percentages with specified digits
    format_percentage <- function(x, d) {
        ifelse(is.na(x) | x == 100, "", sprintf(paste0("%.", d, "f%%"), x))
    }
    ...
    # Modified format_number_x2 to use specified digits
    format_number_x2 <- function(x, d = 1) {
        sapply(x, function(n) {
            if (abs(n - round(n)) < 1e-10) {
                sprintf("%d", round(n))
            } else {
                sprintf(paste0("%.", d, "f"), n)
            }
        })
    }
    ...
    format_cell <- function(obs, exp, row_pct, col_pct, grand_pct, is_total = FALSE) {
        ...
        if (expected && !is_total) {
            ...
            exp_text <- sprintf("(%s)", format_number_x2(exp, digits$ex))
        ...
        if (show_row_pct && !is.na(row_pct)) {
            pct_text <- format_percentage(row_pct, digits$row_pct)
            if (pct_text != "") {
                row_pct_ctx <- list(
                    value = row_pct,
        ...
        if (show_col_pct && !is.na(col_pct)) {
            pct_text <- format_percentage(col_pct, digits$col_pct)
            if (pct_text != "") {
                col_pct_ctx <- list(
                    value = col_pct,
                    formatted_text = pct_text,
        ...
        if (show_total_pct && !is.na(grand_pct)) {
            pct_text <- format_percentage(grand_pct, digits$total_pct)
            if (pct_text != "") {
                total_pct_ctx <- list(
                    value = grand_pct,
                    formatted_text = pct_text,
